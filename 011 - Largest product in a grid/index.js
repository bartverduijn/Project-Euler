...
function largestGridProduct(arr) {
	...
	// Find horizontal and vertical products
	for (let i = 0; i < len; i += 1) {
		for (let j = 0; j <= len - k; j += 1) {
			let horProduct = 1;
			let verProduct = 1;
			for (let n = j; n < k; n += 1) {
				horProduct *= arr[i][n];
			...
			largestProduct = Math.max(largestProduct, horProduct, verProduct);
		}
	}
	//  Search diagonally
	for (let i = 0; i <= len - k; i += 1) {
		for (let j = 0; j <= len - k; j += 1) {
			...
			let diagProduct2 = 1;
			for (let n = 0; n < k; n += 1) {
				diagProduct1 *= arr[i + n][j + n];
				diagProduct2 *= arr[i + n][len - j - n - 1];
			}
